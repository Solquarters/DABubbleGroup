...
export class ChannelService implements OnDestroy, OnInit {
  ...
  private checkWelcomeTeamChannel(): void {
    this.channels$.pipe(first()).subscribe(async (channels) => {
  ...
  private async addUserToWelcomeTeamChannelInFirestore(): Promise<void> {
    ...
    try {
      await updateDoc(channelRef, {
        memberIds: arrayUnion(this.currentUserId),
      });
      console.log(
        `Current user ${this.currentUserId} added to Welcome Team channel in Firestore.`
      );
